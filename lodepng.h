...
/*The PNG color types (also used for raw image_).*/
...
colortype: the desired color type for the raw output image_. See explanation on PNG color types.
bitdepth: the desired bit depth for the raw output image_. See explanation on PNG color types.
...
/*Same as lodepng_decode_memory, but always decodes to 32-bit RGBA raw image_*/
unsigned lodepng_decode32(unsigned char** out, unsigned* w, unsigned* h,
                          const unsigned char* in, size_t insize);

/*Same as lodepng_decode_memory, but always decodes to 24-bit RGB raw image_*/
...
/*Same as lodepng_decode_file, but always decodes to 32-bit RGBA raw image_.
...
/*Same as lodepng_decode_file, but always decodes to 24-bit RGB raw image_.
...
/*
Converts raw pixel data into a PNG image_ in memory. The colortype and bitdepth
  of the output PNG image_ cannot be chosen, they are automatically determined
  by the colortype, bitdepth and content of the input pixel data.
  Note: for 16-bit per channel colors, needs big endian format like PNG does.
out: Output parameter. Pointer to buffer that will contain the PNG image_ data.
     Must be freed after usage with free(*out).
outsize: Output parameter. Pointer to the size in bytes of the out buffer.
image_: The raw pixel data to encode. The size of this buffer should be
       w * h * (bytes per pixel), bytes per pixel depends on colortype and bitdepth.
w: width of the raw pixel data in pixels.
h: height of the raw pixel data in pixels.
colortype: the color type of the raw input image_. See explanation on PNG color types.
bitdepth: the bit depth of the raw input image_. See explanation on PNG color types.
Return value: LodePNG error code (0 means no error).
*/
...
/*Same as lodepng_encode_memory, but always encodes from 32-bit RGBA raw image_.*/
unsigned lodepng_encode32(unsigned char** out, size_t* outsize,
                          const unsigned char* image, unsigned w, unsigned h);

/*Same as lodepng_encode_memory, but always encodes from 24-bit RGB raw image_.*/
...
/*Same as lodepng_encode_file, but always encodes from 32-bit RGBA raw image_.
...
/*Same as lodepng_encode_file, but always encodes from 24-bit RGB raw image_.
...
#ifdef LODEPNG_COMPILE_PNG
/*
Color mode of an image_. Contains all information required to decode the pixel
bits to RGBA colors. This information is the same as used in the PNG file
format, and is used both for PNG and raw image_ data in LodePNG.
*/
...
/*
Check if the given color info indicates the possibility of having non-opaque pixels in the PNG image_.
Returns true if the image_ can have translucent or invisible pixels (it still be opaque if it doesn't use such pixels).
Returns false if the image_ can only have opaque pixels.
In detail, it returns true only if it's a color type with alpha, or has a palette with non-opaque values,
or if "key_defined" is true.
*/
unsigned lodepng_can_have_alpha(const LodePNGColorMode* info);
/*Returns the byte size of a raw image_ buffer with given width, height and color mode*/
...
/*Information about the PNG image_, except pixels, width and height.*/
typedef struct LodePNGInfo {
  ...
  ones defined in the header of the PNG image_, are used.
  ...
  when the image_ is gray, or the compression will be worse since it will be forced to
  ...
  */
  ...
  To avoid this do not set an ICC profile in the image_ unless there is a good reason for it, and when doing so
  ...
  */
  ...
  The color type from the header in the PNG image_ defines these used and unused fields: if
...
typedef enum LodePNGFilterStrategy {
  ...
  on the image_, this is better or worse than minsum.*/
  LFS_ENTROPY,
  /*
...
/*Gives characteristics about the integer RGBA colors of the image_ (count, alpha channel usage, bit depth, ...),
which helps decide which color model to use for encoding.
Used internally by default if "auto_convert" is enabled. Public because it's useful for custom algorithms.*/
typedef struct LodePNGColorStats {
  unsigned colored; /*not grayscale*/
  unsigned key; /*image_ is not opaque and color key is possible instead of full alpha*/
  unsigned short key_r; /*key values, always as 16-bit, in 8-bit case the byte is duplicated, e.g. 65535 means 255*/
  unsigned short key_g;
  unsigned short key_b;
  unsigned alpha; /*image_ is not opaque and alpha channel or alpha palette required*/
  ...
  unsigned allow_greyscale; /*default 1. if 0, choose RGB or RGBA even if the image_ only has gray colors*/
} LodePNGColorStats;

void lodepng_color_stats_init(LodePNGColorStats* stats);

/*Get a LodePNGColorStats of the image_. The stats must already have been inited.
...
typedef struct LodePNGEncoderSettings {
  ...
  the same length as the amount of scanlines in the image_, and each value must <= 5. You
...
typedef struct LodePNGState {
  ...
  LodePNGInfo info_png; /*info of the PNG image_ obtained after decoding*/
  unsigned error;
} LodePNGState;
...
getting much more information about the PNG image_ and color mode.
*/
...
[ ] allow treating some errors like warnings, when image_ is recoverable (e.g. 69, 57, 58)
...
the colors of the raw image_ are always RGB or RGBA, no matter what color type
the PNG file uses. To read and write files, there are simple functions to
convert the files to/from buffers in memory.

This all makes LodePNG suitable for loading textures in games, demos and small
programs, ... It's less suitable for full fledged image_ editors, loading PNGs
over network (it requires all the image_ data to be available before decoding can
...
*) decoding of PNGs with any color type, bit depth and interlace mode, to a 24- or 32-bit color raw image_,
   or the same color type as the PNG
*) encoding of PNGs, from any raw image_ to 24- or 32-bit color, or the same color type as the raw image_
*) Adam7 interlace and deinterlace for any color type
*) loading the image_ from harddisk or decoding it from a buffer from other sources than harddisk
...
Decoding converts a PNG compressed image_ to a raw pixel buffer.
...
*) LodePNGColorMode info_raw: here you can say what color mode of the raw image_ (the output) you want to get
*) LodePNGDecoderSettings decoder: you can specify a few extra settings for the decoder to use

LodePNGInfo info_png
--------------------

After decoding, this contains extra information of the PNG image_, except the actual
pixels, width and height because these are already gotten directly from the decoder
functions.

It contains for example the original color type of the PNG image_, text comments,
...
the resulting raw image_ to be. If this is different from the colortype of the
...
Encoding converts a raw pixel buffer to a PNG compressed image_.
...
since the encoder input is trusted, the decoder input (a PNG image_ that could
be forged by anyone) is not trusted.

When using the LodePNGState, it uses the following fields for encoding:
*) LodePNGInfo info_png: here you specify how you want the PNG (the output) to be.
*) LodePNGColorMode info_raw: here you say what color type of the raw image_ (the input) has
...
You specify the color type of the raw image_ that you give to the input here,
including a possible transparent color key and palette you happen to be using in
your raw image_ data.
...
*) add_id: add text chunk "Encoder: LodePNG <version>" to the image_.
...
the color type of the raw image_, are completely independent. By default, when
you decode a PNG, you get the result as a raw image_ in the color type you want,
no matter whether the PNG was encoded with a palette, grayscale or RGBA color.
And if you encode an image_, by default LodePNG will automatically choose the PNG
color type that gives good compression based on the values of colors and amount
of colors in the image_. It can be configured to let you control it instead as
well, though.
...
By default, when decoding, you get the raw image_ in 32-bit RGBA or 24-bit RGB
color, no matter what color type the PNG has. And by default when encoding,
LodePNG automatically picks the best color model for the output PNG, and expects
the input image_ to be 32-bit RGBA or 24-bit RGB. So, unless you want to control
the color format of the images yourself, you can skip this chapter.

6.1. PNG color types
--------------------

A PNG image_ can have many color types, ranging from 1-bit color to 64-bit color,
as well as palettized color modes. After the zlib decompression and unfiltering
in the PNG image_ is done, the raw pixel data will have that color type and thus
a certain amount of bits per pixel. If you want the output raw image_ after
...
If, when decoding, you want the raw image_ to be something else than the default,
you need to set the color type and bit depth you want in the LodePNGColorMode,
or the parameters colortype and bitdepth of the simple decoding function.

If, when encoding, you use another color type than the default in the raw input
image_, you need to specify its color type and bit depth in the LodePNGColorMode
of the raw image_, or use the parameters colortype and bitdepth of the simple
encoding function.
...
-the decoder converts from PNG to raw image_
-the encoder converts from raw image_ to PNG
-the colortype and bitdepth in LodePNGColorMode info_raw, are those of the raw image_
-the colortype and bitdepth in the color field of LodePNGInfo info_png, are those of the PNG
-when encoding, the color type in LodePNGInfo is ignored if auto_convert
 is enabled, it is automatically generated instead
-when decoding, the color type in LodePNGInfo is set by the decoder to that of the original
 PNG image_, but it can be ignored since the raw image_ has the color type you requested instead
-if the color type of the LodePNGColorMode and PNG image_ aren't the same, a conversion
 between the color types is done if the color types are supported. If it is not
 supported, an error is returned. If the types are the same, no conversion is done.
-even though some conversions aren't supported, LodePNG supports loading PNGs from any
 colortype and saving PNGs to any colortype, sometimes it just requires preparing
 the raw image_ correctly before encoding.
...
The raw input image_ you give to the encoder, and the raw output image_ you get from the decoder
will NOT have these padding bits, e.g. in the case of a 1-bit image_ with a width
...
If you have a buffer containing the PNG image_ data, then the first chunk (the
...
chunk, unsafe to copy chunks of the old image_ may NOT be saved in the new one if your
...
int main(int argc, char *argv[]) {
  ...
  unsigned error = lodepng::decode(image_, width, height, filename);

  //if there's an error, display it
  if(error) std::cout << "decoder error " << error << ": " << lodepng_error_text(error) << std::endl;

  //the pixels are now in the vector "image_", 4 bytes per pixel, ordered RGBARGBA..., use it as texture, draw it, ...
}
...
int main(int argc, char *argv[]) {
  ...
  error = lodepng_decode32_file(&image_, &width, &height, filename);
...
state.info_raw.colortype: desired color type for decoded image_
state.info_raw.bitdepth: desired bit depth for decoded image_
...
state.info_raw.colortype: color type of raw input image_ you provide
state.info_raw.bitdepth: bit depth of raw input image_ you provide
state.info_raw: more color settings, see struct LodePNGColorMode
...
*) 6 nov 2011: (!) By default, the encoder now automatically chooses the best
    ...
    raw image_. For this, autoLeaveOutAlphaChannel replaced by auto_choose_color.
...
*) 09 okt 2006: Encoder class added. It encodes a valid PNG image_ from the
    given image_ buffer, however for now it's not compressed.
*) 08 sep 2006: (!) Changed to interface with a Decoder class
*) 30 jul 2006: (!) LodePNG_InfoPng , width and height are now retrieved in different
    way. Renamed decodePNG to decodePNGGeneric.
*) 29 jul 2006: (!) Changed the interface: image_ info is now returned as a
...
LodePNG. If you encounter a PNG image_ that doesn't work properly with this
