...
/*The PNG color types (also used for raw _image).*/
...
colortype: the desired color type for the raw output _image. See explanation on PNG color types.
bitdepth: the desired bit depth for the raw output _image. See explanation on PNG color types.
...
/*Same as lodepng_decode_memory, but always decodes to 32-bit RGBA raw _image*/
unsigned lodepng_decode32(unsigned char** out, unsigned* w, unsigned* h,
                          const unsigned char* in, size_t insize);

/*Same as lodepng_decode_memory, but always decodes to 24-bit RGB raw _image*/
...
/*Same as lodepng_decode_file, but always decodes to 32-bit RGBA raw _image.
...
/*Same as lodepng_decode_file, but always decodes to 24-bit RGB raw _image.
...
/*
Converts raw pixel data into a PNG _image in memory. The colortype and bitdepth
  of the output PNG _image cannot be chosen, they are automatically determined
  by the colortype, bitdepth and content of the input pixel data.
  Note: for 16-bit per channel colors, needs big endian format like PNG does.
out: Output parameter. Pointer to buffer that will contain the PNG _image data.
     Must be freed after usage with free(*out).
outsize: Output parameter. Pointer to the size in bytes of the out buffer.
_image: The raw pixel data to encode. The size of this buffer should be
       w * h * (bytes per pixel), bytes per pixel depends on colortype and bitdepth.
w: width of the raw pixel data in pixels.
h: height of the raw pixel data in pixels.
colortype: the color type of the raw input _image. See explanation on PNG color types.
bitdepth: the bit depth of the raw input _image. See explanation on PNG color types.
Return value: LodePNG error code (0 means no error).
*/
...
/*Same as lodepng_encode_memory, but always encodes from 32-bit RGBA raw _image.*/
unsigned lodepng_encode32(unsigned char** out, size_t* outsize,
                          const unsigned char* image, unsigned w, unsigned h);

/*Same as lodepng_encode_memory, but always encodes from 24-bit RGB raw _image.*/
...
/*Same as lodepng_encode_file, but always encodes from 32-bit RGBA raw _image.
...
/*Same as lodepng_encode_file, but always encodes from 24-bit RGB raw _image.
...
#ifdef LODEPNG_COMPILE_PNG
/*
Color mode of an _image. Contains all information required to decode the pixel
bits to RGBA colors. This information is the same as used in the PNG file
format, and is used both for PNG and raw _image data in LodePNG.
*/
...
/*
Check if the given color info indicates the possibility of having non-opaque pixels in the PNG _image.
Returns true if the _image can have translucent or invisible pixels (it still be opaque if it doesn't use such pixels).
Returns false if the _image can only have opaque pixels.
In detail, it returns true only if it's a color type with alpha, or has a palette with non-opaque values,
or if "key_defined" is true.
*/
unsigned lodepng_can_have_alpha(const LodePNGColorMode* info);
/*Returns the byte size of a raw _image buffer with given width, height and color mode*/
...
/*Information about the PNG _image, except pixels, width and height.*/
typedef struct LodePNGInfo {
  ...
  ones defined in the header of the PNG _image, are used.
  ...
  when the _image is gray, or the compression will be worse since it will be forced to
  ...
  */
  ...
  To avoid this do not set an ICC profile in the _image unless there is a good reason for it, and when doing so
  ...
  */
  ...
  The color type from the header in the PNG _image defines these used and unused fields: if
...
typedef enum LodePNGFilterStrategy {
  ...
  on the _image, this is better or worse than minsum.*/
  LFS_ENTROPY,
  /*
...
/*Gives characteristics about the integer RGBA colors of the _image (count, alpha channel usage, bit depth, ...),
which helps decide which color model to use for encoding.
Used internally by default if "auto_convert" is enabled. Public because it's useful for custom algorithms.*/
typedef struct LodePNGColorStats {
  unsigned colored; /*not grayscale*/
  unsigned key; /*_image is not opaque and color key is possible instead of full alpha*/
  unsigned short key_r; /*key values, always as 16-bit, in 8-bit case the byte is duplicated, e.g. 65535 means 255*/
  unsigned short key_g;
  unsigned short key_b;
  unsigned alpha; /*_image is not opaque and alpha channel or alpha palette required*/
  ...
  unsigned allow_greyscale; /*default 1. if 0, choose RGB or RGBA even if the _image only has gray colors*/
} LodePNGColorStats;

void lodepng_color_stats_init(LodePNGColorStats* stats);

/*Get a LodePNGColorStats of the _image. The stats must already have been inited.
...
typedef struct LodePNGEncoderSettings {
  ...
  the same length as the amount of scanlines in the _image, and each value must <= 5. You
...
typedef struct LodePNGState {
  ...
  LodePNGInfo info_png; /*info of the PNG _image obtained after decoding*/
  unsigned error;
} LodePNGState;
...
getting much more information about the PNG _image and color mode.
*/
...
[ ] allow treating some errors like warnings, when _image is recoverable (e.g. 69, 57, 58)
...
the colors of the raw _image are always RGB or RGBA, no matter what color type
the PNG file uses. To read and write files, there are simple functions to
convert the files to/from buffers in memory.

This all makes LodePNG suitable for loading textures in games, demos and small
programs, ... It's less suitable for full fledged _image editors, loading PNGs
over network (it requires all the _image data to be available before decoding can
...
*) decoding of PNGs with any color type, bit depth and interlace mode, to a 24- or 32-bit color raw _image,
   or the same color type as the PNG
*) encoding of PNGs, from any raw _image to 24- or 32-bit color, or the same color type as the raw _image
*) Adam7 interlace and deinterlace for any color type
*) loading the _image from harddisk or decoding it from a buffer from other sources than harddisk
...
Decoding converts a PNG compressed _image to a raw pixel buffer.
...
*) LodePNGColorMode info_raw: here you can say what color mode of the raw _image (the output) you want to get
*) LodePNGDecoderSettings decoder: you can specify a few extra settings for the decoder to use

LodePNGInfo info_png
--------------------

After decoding, this contains extra information of the PNG _image, except the actual
pixels, width and height because these are already gotten directly from the decoder
functions.

It contains for example the original color type of the PNG _image, text comments,
...
the resulting raw _image to be. If this is different from the colortype of the
...
Encoding converts a raw pixel buffer to a PNG compressed _image.
...
since the encoder input is trusted, the decoder input (a PNG _image that could
be forged by anyone) is not trusted.

When using the LodePNGState, it uses the following fields for encoding:
*) LodePNGInfo info_png: here you specify how you want the PNG (the output) to be.
*) LodePNGColorMode info_raw: here you say what color type of the raw _image (the input) has
...
You specify the color type of the raw _image that you give to the input here,
including a possible transparent color key and palette you happen to be using in
your raw _image data.
...
*) add_id: add text chunk "Encoder: LodePNG <version>" to the _image.
...
the color type of the raw _image, are completely independent. By default, when
you decode a PNG, you get the result as a raw _image in the color type you want,
no matter whether the PNG was encoded with a palette, grayscale or RGBA color.
And if you encode an _image, by default LodePNG will automatically choose the PNG
color type that gives good compression based on the values of colors and amount
of colors in the _image. It can be configured to let you control it instead as
well, though.
...
By default, when decoding, you get the raw _image in 32-bit RGBA or 24-bit RGB
color, no matter what color type the PNG has. And by default when encoding,
LodePNG automatically picks the best color model for the output PNG, and expects
the input _image to be 32-bit RGBA or 24-bit RGB. So, unless you want to control
the color format of the images yourself, you can skip this chapter.

6.1. PNG color types
--------------------

A PNG _image can have many color types, ranging from 1-bit color to 64-bit color,
as well as palettized color modes. After the zlib decompression and unfiltering
in the PNG _image is done, the raw pixel data will have that color type and thus
a certain amount of bits per pixel. If you want the output raw _image after
...
If, when decoding, you want the raw _image to be something else than the default,
you need to set the color type and bit depth you want in the LodePNGColorMode,
or the parameters colortype and bitdepth of the simple decoding function.

If, when encoding, you use another color type than the default in the raw input
_image, you need to specify its color type and bit depth in the LodePNGColorMode
of the raw _image, or use the parameters colortype and bitdepth of the simple
encoding function.
...
-the decoder converts from PNG to raw _image
-the encoder converts from raw _image to PNG
-the colortype and bitdepth in LodePNGColorMode info_raw, are those of the raw _image
-the colortype and bitdepth in the color field of LodePNGInfo info_png, are those of the PNG
-when encoding, the color type in LodePNGInfo is ignored if auto_convert
 is enabled, it is automatically generated instead
-when decoding, the color type in LodePNGInfo is set by the decoder to that of the original
 PNG _image, but it can be ignored since the raw _image has the color type you requested instead
-if the color type of the LodePNGColorMode and PNG _image aren't the same, a conversion
 between the color types is done if the color types are supported. If it is not
 supported, an error is returned. If the types are the same, no conversion is done.
-even though some conversions aren't supported, LodePNG supports loading PNGs from any
 colortype and saving PNGs to any colortype, sometimes it just requires preparing
 the raw _image correctly before encoding.
...
The raw input _image you give to the encoder, and the raw output _image you get from the decoder
will NOT have these padding bits, e.g. in the case of a 1-bit _image with a width
...
If you have a buffer containing the PNG _image data, then the first chunk (the
...
chunk, unsafe to copy chunks of the old _image may NOT be saved in the new one if your
...
int main(int argc, char *argv[]) {
  ...
  //the pixels are now in the vector "_image", 4 bytes per pixel, ordered RGBARGBA..., use it as texture, draw it, ...
}
...
int main(int argc, char *argv[]) {
  ...
  error = lodepng_decode32_file(&_image, &width, &height, filename);
...
state.info_raw.colortype: desired color type for decoded _image
state.info_raw.bitdepth: desired bit depth for decoded _image
...
state.info_raw.colortype: color type of raw input _image you provide
state.info_raw.bitdepth: bit depth of raw input _image you provide
state.info_raw: more color settings, see struct LodePNGColorMode
...
*) 6 nov 2011: (!) By default, the encoder now automatically chooses the best
    ...
    raw _image. For this, autoLeaveOutAlphaChannel replaced by auto_choose_color.
...
*) 09 okt 2006: Encoder class added. It encodes a valid PNG _image from the
    given _image buffer, however for now it's not compressed.
*) 08 sep 2006: (!) Changed to interface with a Decoder class
*) 30 jul 2006: (!) LodePNG_InfoPng , width and height are now retrieved in different
    way. Renamed decodePNG to decodePNGGeneric.
*) 29 jul 2006: (!) Changed the interface: _image info is now returned as a
...
LodePNG. If you encounter a PNG _image that doesn't work properly with this
